...
public class NPCState
{
    ...
    public GameObject GameObject { get; set; } // 추가된 부분
        ...
        Dictionary<string, object> stateData = null)
    {
        ...
        StateData = stateData != null ? new Dictionary<string, object>(stateData) : new Dictionary<string, object>();
    }

    public NPCState Copy()
    {
        var roundedResources = Resources.ToDictionary(kvp => kvp.Key, kvp => (float)Math.Round(kvp.Value, 2));
        return new NPCState(
    ...
    public override bool Equals(object obj)
    {
        ...
        {
            var thisResources = Resources.Where(kvp => kvp.Key != "time" && kvp.Key != "health" && kvp.Key != "mental")
                                         .OrderBy(kvp => kvp.Key)
                                         .ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
            var otherResources = other.Resources.Where(kvp => kvp.Key != "time" && kvp.Key != "health" && kvp.Key != "mental")
                                               .OrderBy(kvp => kvp.Key)
                                               .ToDictionary(kvp => kvp.Key, kvp => kvp.Value);

            return UpperBody.OrderBy(kvp => kvp.Key).SequenceEqual(other.UpperBody.OrderBy(kvp => kvp.Key)) &&
                   LowerBody.OrderBy(kvp => kvp.Key).SequenceEqual(other.LowerBody.OrderBy(kvp => kvp.Key)) &&
                   thisResources.OrderBy(kvp => kvp.Key).SequenceEqual(otherResources.OrderBy(kvp => kvp.Key)) &&
                   Inventory.SequenceEqual(other.Inventory) &&
                   StateData.OrderBy(kvp => kvp.Key).SequenceEqual(other.StateData.OrderBy(kvp => kvp.Key));
        }
        return false;
    }
    ...
    public override int GetHashCode()
    {
        ...
        foreach (var kvp in LowerBody.OrderBy(kvp => kvp.Key))
            hash ^= kvp.Key.GetHashCode() ^ kvp.Value.GetHashCode();
        foreach (var kvp in Resources.Where(kvp => kvp.Key != "time" && kvp.Key != "health" && kvp.Key != "mental").OrderBy(kvp => kvp.Key))
            hash ^= kvp.Key.GetHashCode() ^ kvp.Value.GetHashCode();
