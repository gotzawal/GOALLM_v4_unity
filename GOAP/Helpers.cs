...
public static class Helpers
{
    public static string MakeHashable(object obj)
    {
        ...
        {
            return "{" + string.Join(",", dict.OrderBy(kvp => kvp.Key).Select(kvp => $"{kvp.Key}:{MakeHashable(kvp.Value)}")) + "}";
        }
        else if (obj is Dictionary<string, Place> placeDict)
        {
            return "{" + string.Join(",", placeDict.OrderBy(kvp => kvp.Key).Select(kvp => $"{kvp.Key}:{MakeHashable(kvp.Value)}")) + "}";
        }
        else if (obj is Dictionary<string, Item> itemDict)
        {
            return "{" + string.Join(",", itemDict.OrderBy(kvp => kvp.Key).Select(kvp => $"{kvp.Key}:{MakeHashable(kvp.Value)}")) + "}";
        }
