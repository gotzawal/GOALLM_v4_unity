...
public class GOAPPlanner
{
    ...
    public List<GOAPAction> Plan(NPCState npcState, WorldState worldState)
    {
        ...
        openList.Enqueue(new StateNode(npcState, worldState, initialAchievedGoals, 0, initialHeuristic), initialHeuristic);
        ...
        while (!openList.IsEmpty())
        {
            ...
            {
                Debug.LogError("GOAP Planner exceeded maximum iterations (5000). No valid plan found.");
                return null;
            }

            var currentNode = openList.Dequeue();
            string currentKey = GenerateStateKey(currentNode.NPCState, currentNode.WorldState, currentNode.AchievedGoals);
            ...
            bool allGoalsMet = goals.All(goal => goal.Condition(currentNode.NPCState, currentNode.WorldState) || goal.Weight <= 0);
            ...
            foreach (var action in actions)
            {
                if (action.IsApplicable(currentNode.NPCState, currentNode.WorldState))
                {
                    ...
                    var (newNpcState, newWorldState) = action.Apply(currentNode.NPCState, currentNode.WorldState);
                    ...
                    {
                        if (!newAchievedGoals.Contains(goal.Name) && goal.Condition(newNpcState, newWorldState))
                        {
                    ...
                    if (!costSoFar.ContainsKey(newKey) || newCost < costSoFar[newKey])
                    {
                        ...
                        openList.Enqueue(new StateNode(newNpcState, newWorldState, newAchievedGoals, newCost, h), priority);
        ...
        Debug.LogError("GOAP Planner could not find a plan to achieve the goals within iteration limits.");
        return null;
    }
    ...
    private float Heuristic(NPCState npcState, WorldState worldState, HashSet<string> achievedGoals)
    {
        ...
        return goals.Where(goal => !achievedGoals.Contains(goal.Name) && goal.Weight > 0)
    ...
    private class StateNode
    {
        ...
        public StateNode(NPCState npcState, WorldState worldState, HashSet<string> achievedGoals, float costSoFar, float heuristic)
        {
            NPCState = npcState;
